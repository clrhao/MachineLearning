...
def Bayes(data, p, avg, var): # data为数组类型
    # print(type(data), type(avg), type(var))
    result = p
...
def confuse_maxtria(predict, fact):
    ...
    for i in range(len(predict)):
        ls[fact[i] - 1][predict[i] - 1] += 1 # 因为索引从 0 开始，而假设的类别从 1 开始
    return ls
...
def get_feature(confuse_maxtria):
    for index in range(len(confuse_maxtria)):
        ...
        for i in range(len(confuse_maxtria)):
            total += confuse_maxtria[index][i] # 遍历当前行，求和
        for i in range(len(confuse_maxtria)):
            total2 += confuse_maxtria[i][index] # 遍历当前列，求和
